...
typedef struct 
{
    unsigned short    nLogicIdx;         // 로직인덱스 번호 (0 ~ 65535)
                                         //  0  ~ 511  : reserved (연동장치 상태 등)
                                         // 512 ~ 8191 : 취급변수 (CMD)
    ...
    UCHAR   TimerKind;         // 2 : 낙하상태에서 조건 만족시 시간동안 delay 후 여자)
                               // 3 : 여자상태에서 조건 만족시 시간동안 delay 후 낙하)
                               // 4 : 여자 조건일 때 delay 후 여자
                               //     낙하 조건일 때 delay 후 낙하
                               // 5 : 조건이 만족하는 동안 dleay 시간에 대한 여자와 낙하 반복
                               // 6 : 조건이 만족할 때 여자동작을 시작하고, delay 시간 후 낙하
